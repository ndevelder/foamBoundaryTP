    ...
    forAll(nutw, faceI)
    {
		...
		if(pswType_ == "nutplus"){	
            if(kPlus <= 2.25){
    			...
    			psw[faceI] = (1.0 + cr_*1.5 + cr_*(pow(90.0/kPlus, 0.5)))*(0.21*tpr.boundaryField()[patchI][faceI]*(kr.boundaryField()[patchI][faceI])/(epsr.boundaryField()[patchI][faceI] + SMALL))*vort.boundaryField()[patchI][faceI];
    		} 
        } 
